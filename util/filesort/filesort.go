...
var g_sc *variable.StatementContext
var g_byDesc []bool
...
func (rh *rowHeap) Less(i, j int) bool {
	...
	ret, err := lessThan(g_sc, l, r, g_byDesc)
	if rh.err == nil {
		rh.err = err
	}
	return ret
...
func (w *FileSortWorker) Len() int { return len(w.buf) }

func (w *FileSortWorker) Swap(i, j int) { w.buf[i], w.buf[j] = w.buf[j], w.buf[i] }

func (w *FileSortWorker) Less(i, j int) bool {
	l := w.buf[i].key
	r := w.buf[j].key
	// TODO: handle error here
	ret, _ := lessThan(g_sc, l, r, g_byDesc)
	return ret
}

func (w *FileSortWorker) input(row *comparableRow) error {
...
func (w *FileSortWorker) flushToFile() {
	defer w.ctx.wg.Done()
	var (
		err        error
		outputFile *os.File
...
func (b *Builder) Build() (*FileSorter, error) {
	...
	ws := make([]*FileSortWorker, nWorkers)
	for i := range ws {
		ws[i] = &FileSortWorker{
			keySize: b.keySize,
			valSize: b.valSize,
